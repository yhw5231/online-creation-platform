...
go mod download
./build.sh                 # 优先读取 VERSION 文件注入版本号，产出 app
                           # （Windows 用 build.bat，产出 app.exe；无 VERSION 时回退 git 标签/开发版本号）
mkdir -p data static
SESSION_SECRET=xxx ./app # 或 Windows: $env:SESSION_SECRET="xxx"; .\app.exe
```

> 版本号说明：仓库根目录 `VERSION` 文件是唯一版本来源，每次推送到 master 时 CI 自动把 `VERSION` 与 `main.go` 内置默认版本号（`AppVersion`）同步 +1 并回写仓库；发布（ghcr 镜像 / 原生安装包）时用 `-ldflags "-X main.AppVersion=vX.Y.Z"` 注入同一版本号，保证镜像内版本号与文件内版本号一致（详见 `docs/release.md`）。本地构建请使用 `build.sh` / `build.bat`，脚本优先读取 `VERSION` 文件，避免系统内版本号与发布版本不一致。
