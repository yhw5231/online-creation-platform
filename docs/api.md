...
| `channel` | string | 否 | 渠道**稳定编号**（字符串，从 1 开始，如 `"1"`），留空自动选第一个普通渠道；编号在渠道创建时分配、不随增删/排序变化，获取方式见 [查询可用渠道](#4-查询可用渠道)；编号无效返回 400 |
...
> 注意：`channel` 是**字符串类型**，传数字（如 `1`）会导致整包 JSON 解析失败返回 `400 invalid json`。
> `channel` 必须是渠道的**稳定编号**（而不是数组下标）：编号在渠道创建时分配并持久化，渠道增删、调整顺序都不会改变已有编号。若提交的是不存在的编号，返回 `400` 并提示查询渠道列表。
> 返回格式由系统后台自适应固定（URL），请求体中**不需要**（也不接受）`response_format` 字段。

示例：

```json
{"prompt": "一只猫，在窗台晒太阳", "channel": "1", "n": 2, "aspect_ratio": "1:1", "resolution": "2k"}
```
...
`GET /api/v1/channels`（需要 API Key）：返回当前配置的全部生成渠道与各自的稳定编号。
...
- `id`：渠道**稳定编号**，即 `channel` 参数的取值（示例中 NSFW 渠道 id 为 3，说明中间曾有渠道被删除，编号不复用）。
- `index`：仅为当前列表的**展示顺序**，会随渠道增删变化，**不要**用作 `channel` 参数。
- 创作页面「生成渠道」下拉也会显示「编号 · 渠道名」，看到的编号与接口返回的 `id` 完全一致。
...
# 0. 查询可用渠道（channel 参数的编号取自返回的 id 字段）
...
curl -X POST "$BASE/api/v1/generate" \
  ...
  -d '{"prompt":"一只猫","n":1,"channel":"1","aspect_ratio":"1:1","resolution":"2k"}'
...
| 400 | `{"ok":false,"error":"渠道编号无效：…"}` | channel 提交的编号不存在（查询渠道列表见第 4 节） |
| 400 | `{"ok":false,"error":"所选渠道不支持 2k 分辨率，支持：1k, 4k"}` | 分辨率不属于该渠道 |
...
检查请求体是否合法 JSON，且所有字段类型正确——尤其是 `channel` 必须传字符串 `"1"` 而不是数字 `1`。

**Q：`channel` 参数该填什么？从哪里查？**
填渠道的**稳定编号**（如 `"1"`）：编号在渠道创建时分配、不随增删/排序变化。两个获取途径：① 打开创作页面，「生成渠道」下拉显示「编号 · 渠道名」；② 调用 `GET /api/v1/channels`（见第 4 节）读取每个渠道的 `id` 字段。不要用列表顺序（`index`）当编号，渠道增删后顺序会变。
